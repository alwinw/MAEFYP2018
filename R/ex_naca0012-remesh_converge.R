...
#--- * Required Paths                                             ----
if (FALSE) {                                                    # Test cases
  saveplot   = "../src-example/NACA0012_remesh_convergence/"             # Pass in later to plot_data
  airfoil    = "NACA0012"
  folderpath = "../src-example/NACA0012_remesh_convergence/NACA0012-N_P04/"
...
batchfolder = "../src-example/NACA0012_remesh_convergence"
...
df_dump <- filter(df_dump, !is.na(tokenvalue)) # SOME PATCH BECAUSE N_P APPEARS MORE THAN ONCE IN THE FILE!!
...
ggplot(conv$wall, aes(s)) +
  ...
  ggtitle(paste("N-S Vorticity Generation N_P =", sample)) +
  ylim(-40, 20)
...
conv$fld <- outp$node %>% 
  filter(local != 1 & x <= 0.55)%>%
  select(tokenvalue, u, v, p, dodx, dody, dpdx, dpdy, o) 
# Determine max(N_P) then set that data to N_P = 0 as it is the reference
maxNP <- max(conv$fld$tokenvalue)
conv_maxN <- filter(conv$fld, tokenvalue == maxNP) %>% 
  mutate(tokenvalue = 0)
# Lapply get the error
conv_err  <- split(conv$fld, conv$fld$tokenvalue)
conv_err  <- lapply(conv_err, function(df) {
  tokenvalue = df$tokenvalue
  df$tokenvalue <- NULL
  conv_maxN$tokenvalue  <- NULL
  out = cbind(tokenvalue,
              abs((df-conv_maxN)))
  return(out)
})
...
ggplot(conv_flderrplot, aes(tokenvalue, norm_inf, group = var, colour = var)) +
  ...
  scale_colour_manual(
    labels = c("u", "v", "p", 
               ...
               expression(frac(d*omega, d*x)), expression(frac(d*omega, d*x))),
    values = brewer.pal(9, "Set2")
  ) +
