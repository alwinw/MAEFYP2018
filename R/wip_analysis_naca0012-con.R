...
data_batch <- ListSesh(batchfolder)
# List of unique airfoils & N_P
list_airfoil <- split (data_batch, data_batch$airfoil)
list_airfoil <- lapply(list_airfoil, function(df) df[1,])
outp_airfoil <- lapply(list_airfoil, BatchLoadAirfoil)
# List of meshs (should turn this into a function!)
tokenwords = list("N_P")                                        # Find unique bndry and knot N values
data_mesh <- data_batch %>%
  rowwise() %>%
...
list_mesh <- split (data_mesh, data_mesh$ID)                    # Unique airfoil and N_P
list_mesh <- lapply(list_mesh, function(x) x[1,])               # Take only the first row in each
outp_mesh <- lapply(list_mesh, BatchLoadMesh, outp_airfoil)
