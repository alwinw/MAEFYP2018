...
void vor2Dl(Point  CP ,	/* Collocation point                           */
	    Point  LE ,	/* Panel leading edge coordinates              */
	    Point  TE ,	/* Panel trailing edge coordinates             */
	    Point  n  ,	/* Components of panel unit outward normal     */
	    Point *Va ,	/* Velocity components in global system        */
	    Point *Vb ,	/*                                             */
            int    IN   /* Collocation point interior to panel if != 0 */ )
/* =========================================================================
 * Influence coefficient calculation for velocity components measured
 * in global coordinate system on the assumption of linear vorticity
 * distribution over the element.  As discussed in Katz & Plotkin \S
 * 11.4.2, velocity components are split up according to the panel-end
 * vorticity strengths: this facilitates matrix solution for the
 * unknowns (which are just these panel-end strengths).
 *
 ...
 * ========================================================================= */
{
  ...
  // Translation of TE, LE and CP to panel co-ordinates.
  TE.x -= LE.x;  TE.z -= LE.z;
  CP.x -= LE.x;  CP.z -= LE.z;
  LE.x  = 0.0;   LE.z  = 0.0;
  ...
  // Rotation of CP to panel co-ordinates.
