...
#--- Inputs ----
input <- list(
  x = data.frame(min = -1.75, max = 5.4375, n = 21), #51
  y = data.frame(min = -1.25, max = 1.25,   n = 11), #21
  t = 0.12,
  alpha = 4, # degrees
  polyn = 2,
  NACA = 2412
...
if(FALSE) {
  airfoil <- data.frame(x = seq(0.5, -0.5, -0.001))
...
AirfoilTrans <- function(n1) {
  if (n1$bcx == "CD") {
    ...
    if (n1$bcy == "UP" | n1$bcy == "CD") {
      x0 = Airfoilx(input$NACA, x, "upper")
      ys = AirfoilCurve(input$NACA, x0)$yU
      dy = ys*(cos(pi/(2*(ym-y0))*(y-y0))^2)
      n1$y = n1$y + dy
    }
    if (n1$bcy == "LO") {
      x0 = Airfoilx(input$NACA, x, "lower")
      ys = AirfoilCurve(input$NACA, x0)$yL
      dy = ys*(cos(pi/(2*(ym-y0))*(y+y0))^2)
      n1$y = n1$y + dy
    }
  }
...
for (j in 1:(ny - 1)) {
  for (i in 1:(nx - 1)) {
    ...
    # Transform y for the airfoil thickness
    # n1 = AirfoilTrans(n1)
    # n2 = AirfoilTrans(n2)
    # n3 = AirfoilTrans(n3)
    # n4 = AirfoilTrans(n4)
...
# 
# ggplot(dfelem %>% filter(i <= 10, j <= 8), aes(x, y, group = enum)) +
#   geom_polygon(aes(colour = enum), fill = NA) +
#   coord_fixed(xlim = c(-0.5, 0.5), ylim = c(-0.3, 0.3)) + 
#   scale_colour_gradientn(colours = spectralpalette(10))
